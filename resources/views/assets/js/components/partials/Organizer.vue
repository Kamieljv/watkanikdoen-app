  ...
  <a :href="route" :title="titleText" @click="clickOrganizer">
    <div
      ...
      :class="{
        ...
        'text-white bg-blue-600 hover:bg-blue-500 focus:outline-none focus:border-blue-700 active:bg-blue-700':
    ...
    >
      <div class="flex w-full space-x-3 justify-start items-center">
        ...
        <div class="flex flex-1 flex-col overflow-hidden">
          ...
          <p
            v-if="type === 'large'"
            v-sanitize.basic="organizer.description"
            class="text-sm line-clamp-2"
          ></p>
          <div
            v-else-if="type === 'small' && showThemes"
            class="sm:hidden pt-2 flex space-x-1 justify-end"
          >
            <ThemesChips
              :themes="organizer.themes"
              :max-visible="2"
              justify="end"
              :show-plus-label="false"
            />
          </div>
        </div>
      </div>
      <div
        v-if="showThemes"
        class="hidden sm:flex w-full items-center justify-end"
      >
        <ThemesChips :themes="organizer.themes" justify="end" />
      </div>
      <div
        v-if="mode == 'select' && selected"
        class="flex justify-end items-center"
      >
        <CheckIcon
          class="shrink-0"
          style="stroke: currentColor; width: 26px; height: 24px"
        />
      </div>
      <div
        v-else-if="mode == 'remove'"
        class="flex justify-center items-center cursor-pointer rounded-full w-8 h-8 bg-gray-100 hover:bg-gray-200"
        @click="toggleSelect"
      >
        <DeleteIcon
          class="shrink-0"
          style="stroke: currentColor; width: 24px; height: 24px"
        />
      </div>
    </div>
  </a>
