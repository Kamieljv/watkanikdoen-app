<template>
  <article
    ...
    typeof="Article"
  >
    <meta property="name" :content="props.referentie.title" />
    <meta property="author" typeof="Person" content="admin" />
    <meta
      property="dateModified"
      :content="new Date(props.referentie.updated_at).toISOString()"
    />
    <meta
      class="uk-margin-remove-adjacent"
      property="datePublished"
      :content="new Date(props.referentie.created_at).toISOString()"
    />
    <div class="content flex flex-col">
      <div class="flex-shrink-0" style="position: relative">
        <img
          v-if="props.referentie.image_url"
          class="object-cover w-full h-[150px]"
          :src="props.referentie.image_url"
          alt=""
        />
        <div
          v-else
        ...
        <ul
          v-if="props.referentie.themes.length <= 2"
          class="themes-container flex flex-wrap p-2 absolute top-0 w-full"
        >
          <li
            v-for="theme in props.referentie.themes"
            :key="theme.id"
        ...
        >
          <li
            ...
            :style="{ backgroundColor: props.referentie.themes[0].color }"
          >
            <span class="text-white" rel="theme">
              {{ props.referentie.themes[0].name }}
            </span>
          </li>
          <li
            class="relative self-start inline-block px-2 py-1 mr-1 mb-1 text-xs font-medium leading-5 text-white uppercase bg-gray-100 rounded"
          >
            <span class="text-gray-800" rel="theme">
              +{{ props.referentie.themes.length - 1 }} thema's
            </span>
          </li>
        </ul>
      </div>
    ...
    <div class="flex flex-col justify-between flex-1 bg-white">
      <div class="title-body-container p-3 overflow-hidden relative">
        ...
        >
          {{ props.referentie.title }}
        </h3>
        <p
          v-sanitize.inline="props.referentie.description"
