<template>
  <Form ref="organizerValidatorRef">
    <p class="text-body leading-5 text-gray-500 mt">
      Wie is de organisator van je actie? Kies één of meerdere organisatoren uit
      de lijst. Kun je de juiste organisator niet vinden? Dan kun je deze zelf
      toevoegen met het formulier.
    </p>
    <div
      class="grid grid-cols-1 md:grid-cols-5 max-w-6xl mx-auto flex-col my-6 md:divide-x divide-gray-600"
    >
        ...
        >
          ...
          <div class="flex flex-col mt-5 md:pr-5">
            ...
            <Organizers
              ...
              :max="5"
              mode="select"
              @update:model-value="updateOrganizersSelected"
            />
          </div>
        </div>
      </div>
      <div class="col-span-3 space-y-3">
        ...
        >
          <div class="flex flex-col mt-5 md:pl-5">
            ...
            <p class="text-sm leading-5 text-gray-500 mt">
              Omschrijf de organisator. Gebruik daarbij eventueel teksten van
              deze organisator, zolang je vermeldt waar deze vandaan komen.
            </p>
          </div>
          <div class="flex flex-col mt-5 md:pl-5">
            <!-- Name -->
            ...
            <div class="flex mt-5 justify-end">
              <a
                class="primary bg-green-600 hover:bg-green-500 focus:outline-none focus:border-green-700 active:bg-green-700 flex items-center"
                @click="addOrganizer"
              >
                <AddLineIcon
                  class="shrink-0 h-5 w-[17px] mr-[5px]"
      ...
      >
        ...
        <div v-if="organizersSelected.length > 0" class="flex flex-col mt-5">
          <Organizer
            ...
            :disabled="true"
            mode="remove"
            @update:model-value="removeSelected($event, organizer)"
          />
        </div>
...
const addOrganizer = () => {
  ...
  if (
    organizersSelected.value.find(
      (v) => v.name.toLowerCase() === name.value.toLowerCase(),
    )
  ) {
...
onMounted(() => {
  organizersSelected.value =
    props.modelValue.length > 0 ? props.modelValue : [];
});

watch(organizersSelected, () => {
