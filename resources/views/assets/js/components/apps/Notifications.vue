<template>
  <div
    class="flex flex-col justify-start flex-1 mb-5 overflow-hidden bg-white border shadow-md rounded-lg border-gray-200"
  >
    <div
      class="flex flex-wrap items-center justify-between p-5 bg-white border-b border-gray-200 sm:flex-no-wrap"
    >
      <div class="relative flex-1">
        <h3
          class="flex items-center text-lg font-medium leading-6 text-gray-900 space-x-3"
        >
          <span>{{ __("notifications.notifications") }}</span>
          <div
            v-if="unreadNotifications > 0"
            id="notification-count"
            class="flex items-center justify-center w-5 h-5 text-sm font-extrabold text-red-100 bg-red-500 rounded-full"
          >
            {{ unreadNotifications }}
          </div>
        </h3>
    ...
    <div
      ...
      class="flex items-center justify-center h-24 w-full text-gray-600 font-medium"
    >
      <InboxIcon class="w-5 h-5 mr-3" />
      {{ __("dashboard.no_notifications") }}
    </div>

    <div id="notification-ul" class="relative max-h-100 overflow-y-scroll">
      <div
      ...
      >
        <div class="flex items-start p-5">
          <div class="shrink-0 pt-1">
            <div
              class="flex items-center justify-center text-xl w-10 h-10 rounded-full text-gray-400"
              :class="{
                'text-white bg-blue-500': n.unread,
                'bg-gray-100': !n.unread,
              }"
            >
          ...
          <div class="flex flex-col items-start flex-1 w-0 ml-3">
            <a :href="n.data.link">
              <div
                ...
                :class="{ 'text-gray-700': n.unread }"
              >
                <span
                  class="text-[15px] font-medium"
                  :class="{ 'font-black': n.unread }"
                  >{{ n.data.title }}</span
                ><br />
                <span :class="{ 'font-bold': n.unread }">{{
                  n.data.body
                }}</span>
              </div>
            </a>
            ...
            >
              ...
              <span
                ...
                :data-id="n.id"
                :data-listid="i"
                class="flex justify-start text-xs text-gray-500 cursor-pointer hover:text-gray-700 mark-as-read hover:underline"
                @click="markAsRead"
              >
                <svg
                  class="absolute w-4 h-4 mt-1 mr-1"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
...
const notif: Ref<Notification[]> = ref(props.notifications);

const noNotifications = computed(() => notif.value.length === 0);

const notificationsProcessed = computed(() => {
  return notif.value.map((n: Notification) => {
...
const markAsRead = (e) => {
  axios
    .post("auth/notification/read/" + e.target.dataset.id)
    .then((response) => {
      if (response.data.type == "success") {
        notif.value[e.target.dataset.listid].read_at = new Date();
        notif.value[e.target.dataset.listid].unread = false;
      }
    });
};
</script>
