  ...
  <div>
    <div v-if="filterable" class="row mx-auto max-w-6xl">
      <div id="filter-container" class="row my-3">
        ...
        <div
          id="filter-wrapper"
          class="col grid gap-3"
          :class="{ 'grid-cols-2': themes.length > 0 }"
        >
          <FormField
            v-model="query"
            name="query"
        ...
        <div class="relative mx-auto w-full">
          <div class="relative mx-auto max-w-7xl">
            <div
              ...
              class="grid grid-auto gap-5 mx-auto mt-12 sm:grid-cols-2 lg:grid-cols-3"
            >
              <div
              ...
              >
                ...
                <div class="p-3">
                  <div
                    class="relative h-6 w-full inline-block bg-gray-200 rounded"
                  ></div>
                  <div
                    class="relative h-3 w-full inline-block bg-gray-200 rounded"
                  ></div>
                </div>
              </div>
            </div>
            <div
              v-else-if="heeftReferenties"
              class="grid gap-5 mx-auto mt-12 sm:grid-cols-2 lg:grid-cols-3"
            >
              <Referentie
                v-for="referentie in referentiesFormatted"
                :key="referentie.id"
                :referentie="referentie"
                @click="openReferentieModal($event, referentie)"
              />
            </div>
            <div
              v-else-if="isGeladen"
    ...
    >
      <template #header>
        <img
          v-if="currentReferentie.image_url"
          class="object-cover w-full h-[20rem]"
          :src="currentReferentie.image_url"
          alt=""
        />
        <div
          v-else
          class="h-[150px] md:h-[250px] w-full bg-gray-300 text-gray-400 flex items-center justify-center"
        >
      ...
      <div class="title-body-container overflow-hidden relative">
        ...
        <p
          v-sanitize.strip="currentReferentie.description"
...
const referentiesFormatted = computed(() => {
  referenties.value.forEach((referentie) => {
    referentie.description = referentie.description
      ? referentie.description.replace(/(<([^>]+)>)/gi, "")
      : null;
    return referentie;
  });
  ...
  axios
    ...
    .then((response) => {
      if ("per_page" in response.data.referenties) {
        if (appending.value) {
          referenties.value = referenties.value.concat(
            response.data.referenties.data,
          );
        } else {
