        ...
        <ul class="nav navbar-nav @if (__('voyager::generic.is_rtl') == 'true') navbar-left @else navbar-right @endif">
            <li class="dropdown profile">
                <a href="#" class="dropdown-toggle text-right" data-toggle="dropdown" role="button" aria-expanded="false">
                    @if ($user_avatar)
                        <img src="{{ $user_avatar }}" alt="{{ Auth::user()->name }}" class="profile-img">
                    @else
                        <div class="no-profile-img small">{{ strtoupper(substr(Auth::user()->name, 0, 1)) }}</div>
                    @endif
                ...
                <ul class="dropdown-menu dropdown-menu-animated">
                    <li class="profile-img">
                        @if ($user_avatar)
                            <img src="{{ $user_avatar }}" alt="{{ Auth::user()->name }}" class="profile-img">
                        @else
                            <div class="no-profile-img large">{{ strtoupper(substr(Auth::user()->name, 0, 1)) }}</div>
                        @endif
