...
@if(!isset($show_all_notifications))
    ...
    <div id="notification-list" @click.away="open = false" class="header-icon flex items-center h-full"
        x-data="{ open: false }">
        <div id="notification-icon relative">
            <button @click="open = !open"
                class="relative p-1 ml-3 text-gray-500 transition duration-150 ease-in-out rounded-full hover:text-gray-700 focus:outline-none focus:text-gray-700 focus:bg-gray-100">
                ...
                @if($unreadNotifications && $notifications_count > 0) <span id="notification-count"
                    class="absolute top-0 right-0 flex items-center justify-center w-4 h-4 text-xs font-extrabold text-red-100 bg-red-500 rounded-full">{{ $notifications_count }}</span>
                @endif
            </button>
        </div>
@else
    ...
    @if(!isset($show_all_notifications))
        <div x-show="open" x-transition:enter="duration-100 ease-out scale-95"
            ...
            class="absolute top-0 left-2 sm:left-auto right-2 sm:right-0 max-w-lg mt-[70px] sm:mt-[50px] overflow-hidden origin-top-right transform rounded-xl shadow-lg max-w-7xl w-104"
            x-cloak>
    @else
                ...
                <div class="relative">
                    ...
                    @foreach ($unreadNotifications as $index => $notification)
                        ...
                        <div id="notification-li-{{ $index + 1 }}"
                            class="flex flex-col border-b border-gray-200 @if(!isset($show_all_notifications)){{ 'hover:bg-gray-50' }}@endif">
                            <div class="flex items-start p-5">
                                <div
                                    class="flex items-center justify-center text-xl w-10 h-10 rounded-full text-gray-400 bg-gray-100">
                                ...
                                <div class="flex flex-col items-start flex-1 w-0 ml-3">
                                    <a href="{{ @$notification_data->link }}">
                                        <div class="text-sm leading-5 text-gray-700">
                                            <span
                                                class="text-[15px] font-bold">{{ @$notification_data->title }}</span><br />
                                    ...
                                    <div
                                        ...
                                        <span
                                            class="notification-datetime">{{ \Carbon\Carbon::parse(@$notification->created_at)->diffForHumans() }}</span>
                                        <span>•</span>
